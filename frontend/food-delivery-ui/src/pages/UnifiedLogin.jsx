...
export default function UnifiedLogin({ onLogin }) {
  ...
  // Listener to capture backend-generated OTP (fallback for sandbox or transaction requirements)
  useEffect(() => {
    const handleOtpSent = (e) => {
      if (e.detail && e.detail.otp) {
        setDevOtp(e.detail.otp);
      }
    };
        ...
        <div className="backdrop-blur-xl rounded-3xl p-8 shadow-2xl border transition-all duration-300"
          ...
          {step === 3 && (
            <form onSubmit={handleVerifyOtp} className="space-y-5">
              ...
              <div className="p-3 rounded-xl text-xs font-medium animate-zoom-in leading-relaxed space-y-2"
                style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.25)', color: '#86efac' }}>
                <div>
                  <span className="font-extrabold uppercase block mb-0.5">📱 SMS Dispatched</span>
                  OTP has been sent to <strong>+91 {mobile}</strong>.
                </div>
                {devOtp && (
                  <div className="pt-2 border-t border-green-500/20 text-yellow-300 font-semibold">
                    <span className="text-[10px] uppercase font-bold text-orange-500 block mb-0.5">⚠️ SMS Gateway Sandbox / Delay Fallback</span>
                    If SMS is delayed or blocked by DLT policy, enter code: <strong className="text-white bg-slate-800 px-2 py-0.5 rounded text-sm tracking-wider">{devOtp}</strong>
                  </div>
                )}
              </div>
